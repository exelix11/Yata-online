...
namespace PuyoTools.Modules.Compression
{
    public class Lz11Compression : CompressionBase
    {
        ...
        {
            Compress(source, destination, false);
        }

        public void ArchiveNoCompression(Stream source, Stream destination)
        {
            source.Position = 0;
            destination.Position = 0;

            BinaryWriter bin = new BinaryWriter(destination);
            bin.Write(0x11 | (((int)source.Length) << 8));

            byte[] buf = new byte[8];
            while (source.Read(buf, 0, 8) > 0)
            {
                bin.Write((byte)0x00);
                bin.Write(buf);
            }
        ...
        public void Compress(Stream source, Stream destination, bool fast)
        {
            int sourceLength = (int)(source.Length - source.Position);
            ...
            if (fast)
            {
                dictionary.SetWindowSize(3);
                dictionary.SetMaxMatchAmount(3);
            }
            else
            {
                dictionary.SetWindowSize(0x10);
                dictionary.SetMaxMatchAmount(0x10);
            }
